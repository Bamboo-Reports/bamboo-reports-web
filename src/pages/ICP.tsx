...
const icpItems = useCaseItems.filter((item) => item.badge === "ICP");

const ICP = () => {
  useSEO({
    title: "ICP | Bamboo Reports",
    description: "Explore Bamboo Reports ICP personas in one place.",
  });
              ...
              >
                ...
                <div className="relative flex items-start gap-4 lg:gap-6">
                  <div className="mt-1 rounded-2xl border bg-background/80 text-primary p-3.5 shrink-0 transition-transform duration-micro ease-smooth group-hover:scale-105 group-hover:-rotate-3">
                    <icp.icon className="h-6 w-6 lg:h-7 lg:w-7" />
                  </div>
                  <div className="space-y-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">{icp.title}</p>
                    <h2 className="text-2xl lg:text-[2.1rem] font-black leading-tight max-w-4xl">{icp.headline}</h2>
                    <p className="text-base lg:text-lg text-muted-foreground leading-relaxed max-w-4xl">{icp.description}</p>
