...
and show counts:
  ...
  - netlify/functions/_data/tracker-v2-accounts.json
                                 the full account rows plus the meta the
                                 query function needs. Bundled into the
                                 Netlify function; NEVER published, so the
                                 dataset cannot be downloaded wholesale.
  - public/data/t2/*.json        the same rows, chunked and public. Only
                                 written with --emit-public-chunks, because
                                 publishing them lets anyone fetch the whole
                                 dataset and bypass the exposure cap.
...
The tracker page picks its source with VITE_TRACKER_SERVER_QUERY: "true"
queries the function, anything else fetches the public chunks. Regenerate
with --emit-public-chunks before turning that flag off, or the page will
have no chunks to fetch.
"""
...
# Read by netlify/functions/tracker-query.js and bundled into it by esbuild.
# Must never be written anywhere under public/.
SERVER_DATASET = ROOT / "netlify" / "functions" / "_data" / "tracker-v2-accounts.json"
...
def main():
    ...
    unknown = flags - {"--emit-public-chunks"}
    if unknown:
        sys.exit(f"Unknown option(s): {', '.join(sorted(unknown))}")
    emit_public_chunks = "--emit-public-chunks" in flags
    source = Path(args[0]) if args else DEFAULT_INPUT
    ...
    # Public chunks are opt-in: publishing them puts the whole dataset behind
    # a plain URL, which defeats the exposure cap the table below computes.
    # Stale chunks are removed either way, so a regen without the flag takes
    # the previously published files down.
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    for stale in OUTPUT_DIR.glob("*.json"):
        stale.unlink()
    chunk_urls = []
    if emit_public_chunks:
    ...
    CHUNK_MANIFEST.write_text(
        ...
        f"export const TRACKER_V2_ACCOUNT_CHUNKS = [{manifest_body}] as const;\n"
    )
    ...
    updated = date.fromtimestamp(source.stat().st_mtime).strftime("%d %B %Y").lstrip("0")
    ...
    STATS_MODULE.write_text(
        ...
        f'export const TRACKER_V2_UPDATED = "{updated}" as const;\n\n'
    ...
    # The server dataset: everything tracker-query.js needs to answer one
    # filter combination, written where only the function can reach it. The
    # meta block mirrors the constants emitted into trackerStatsV2.ts above,
    # so the function and the client apply the same caps and groupings.
    SERVER_DATASET.parent.mkdir(parents=True, exist_ok=True)
    SERVER_DATASET.write_text(json.dumps({
        "meta": {
            ...
            "nonGccNotes": non_gcc_notes,
        },
        "accounts": tracker_accounts,
    }, ensure_ascii=False, separators=(",", ":")))

    print(f"Wrote {len(tracker_accounts)} accounts to {SERVER_DATASET.relative_to(ROOT)}")
    if emit_public_chunks:
        print(f"Wrote {len(chunk_urls)} PUBLIC chunks to {OUTPUT_DIR.relative_to(ROOT)} "
              f"— the full dataset is downloadable from these URLs")
    else:
        print(f"No public chunks written; {OUTPUT_DIR.relative_to(ROOT)} purged. "
              f"The tracker needs VITE_TRACKER_SERVER_QUERY=true.")
